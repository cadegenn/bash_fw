...
main() {
	...
	help() {
		cat <<- EOF
			DESCRIPTION: ${BASENAME} install tiny Bash Framework system-wide
			USAGE: ${BASENAME} [-h -H] [--prefix /usr]
			 	-h			show help (this screen)
			 	-H			show help (this screen) with common parameters usage
			 	--prefix	path where to install bash_fw [${PREFIX}]
	...
	[ ! -d "${PREFIX}" ] && eexec mkdir -p "${PREFIX}"
	eexec cp -a "${DIRNAME}/*" "${PREFIX}/"
	[ -d /etc/profile.d/ ] && {
		cat <<- EOF > /etc/profile.d/bashfw.sh
			export BASHFW_PATH="${PREFIX}"
		EOF
		eexec chmod 644 /etc/profile.d/bashfw.sh
	}
